      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="rounded-lg border border-border bg-secondary/40 p-4 space-y-4">
          <div>
            ...
            <p className="mt-1 text-muted-ems">
              Either upload a digital file (SHA-256 computed server-side){" "}
              <span className="font-medium text-canvas-foreground">or</span>{" "}
              enter a hash from an external forensic tool (MD5 or SHA-256). One
              of the two is required.
            </p>
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="file">Digital file (optional if hash entered)</Label>
            <Input id="file" name="file" type="file" />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="externalHash">
              Externally computed hash (optional if file uploaded) — MD5 (32
              hex) or SHA-256 (64 hex)
            </Label>
            <Input
              id="externalHash"
              name="externalHash"
              placeholder="e.g. 32-char MD5 or 64-char SHA-256 hex"
