...
export default async function EvidenceListPage({
  ...
  const [items, custodians] = await Promise.all([
  ...
  return (
    <div>
      ...
      <Suspense
        fallback={
          <div className="mb-4 h-32 animate-pulse rounded-lg border border-border bg-card" />
        }
      >
        <EvidenceFilterBar custodians={custodians} />
      </Suspense>
